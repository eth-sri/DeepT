...
class Logger:
    def __init__(self, sess, args, summary_names, key_output_index):
        ...
        self.train_writer = tf1.summary.FileWriter(os.path.join(self.log_dir, "train"))
        self.valid_writer = tf1.summary.FileWriter(os.path.join(self.log_dir, "valid"))
        self.test_writer = tf1.summary.FileWriter(os.path.join(self.log_dir, "test"))
        self.summary_placeholders = [tf1.placeholder(tf.float32) for i in range(self.summary_num)]
        self.summary_op = [tf1.summary.scalar(
        ...
        self.saver = tf1.train.Saver(
            params,
            write_version=tf1.train.SaverDef.V2,
            max_to_keep=None,
