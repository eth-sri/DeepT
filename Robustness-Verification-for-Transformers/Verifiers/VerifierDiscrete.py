    ...
    def verify(self, example):
        ...
        for i in range(1, length - 1):
            ...
            for batch in get_batches(candidates, self.batch_size):
                ...
                dist = torch.norm(r["embedding_output"][:, i] - embeddings[0][i].unsqueeze(0), p=self.p, dim=-1)
            ...
            self.logger.write("Position %d: %s %.5f %.5f" % (i, tokens[i], epsilon, epsilon_normalized))
