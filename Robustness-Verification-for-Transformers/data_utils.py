...
def load_data_yelp(args, set):
    ...
    csv = pandas.read_csv(path, names=["label", "text"])  # CHANGE: I added explicit label
    data = []
    n = csv.shape[0]
    for i in range(n):
        text = csv["text"][i]
...
def sample(args, data, target, num_samples=None):
    examples = []
    num_samples = num_samples or args.samples
    for i in range(num_samples):
        while True:
            example = data[random.randint(0, len(data) - 1)]

            if 'sentence' not in example and args.with_lirpa_transformer:
                example['sentence'] = ' '.join(example['sent_a'])

            if args.with_lirpa_transformer:
                preds, embeddings, tokens = target.step([example])
            else:
                std = target.step([example])[-1]
                embeddings, preds = std["embedding_output"], std["pred_labels"]

            # too long
            if embeddings[0].shape[0] > args.max_verify_length:
                continue
            # incorrectly classified
            if preds[0] != example["label"]:
                continue
...
def compute_accuracy(args, data, target):
    num_examples = 0
    num_correctly_classified_examples = 0
    num_label0 = 0
    num_label1 = 0
    for example in data:
        if 'sentence' not in example:
            assert args.with_lirpa_transformer
            example['sentence'] = ' '.join(example['sent_a'])
        ...
        if args.with_lirpa_transformer:
            ...
            std = target.step([example])[-1]
            embeddings, preds = std["pred_labels"]
        ...
        # incorrectly classified
        if preds[0] == example["label"]:
            num_correctly_classified_examples += 1

    print(f"{num_correctly_classified_examples}/{num_examples} examples classified correctly (num label 0: {num_label0}, num_label 1: {num_label1})")
    return num_examples, num_correctly_classified_examples, num_correctly_classified_examples / num_examples



def get_all_correctly_classified_samples(args, data, target):
    examples = []
    for example in data:
        if 'sentence' not in example:
            assert args.with_lirpa_transformer
