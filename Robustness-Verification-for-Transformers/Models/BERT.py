...
from Models.utils import acc_and_f1, convert_examples_to_features, convert_examples_to_features_word_model
...
class BERT:
    ...
    def get_input(self, batch):
        features = convert_examples_to_features(batch, self.label_list, self.max_seq_length, self.tokenizer)
    ...
    def step(self, batch, is_train=False, infer_grad=False, embeddings=None, capture_values=False, custom_loss=None):
        input_ids, input_mask, segment_ids, features = self.get_input(batch)
        label_ids = torch.tensor([f.label_id for f in features], dtype=torch.long)
        label_ids = label_ids.to(self.device)

        values = [] if capture_values else None

        if is_train:
            self.model.train()
            logits, embedding_output, encoded_layers, attention_scores, attention_probs, \
            self_output, pooled_output = self.model(input_ids, segment_ids, input_mask,
                                                    labels=None, embeddings=embeddings, values_storage=values)
        else:
            self.model.eval()
            ...
            with grad:
                ...
                self_output, pooled_output = self.model(
                    input_ids, segment_ids, input_mask, labels=None, embeddings=embeddings, values_storage=values)
...
class BertWithWordEmbeddings(BERT):
    def __init__(self, args, data_train, word_embeddings, vocab):
        super(BertWithWordEmbeddings, self).__init__(args, data_train)

        # Replace word embeddings
        self.model.bert.embeddings.word_embeddings = word_embeddings

        # Delete default tokenizer and use custom vocab instead
        del self.tokenizer
        self.vocab = vocab

    def get_input(self, batch: List[Mapping]):
        features = convert_examples_to_features_word_model(batch, self.label_list, self.max_seq_length, self.vocab, drop_unk=False)
